...
// ─── Internal state ────────────────────────────────────────────────────────────
// Tracks whether each low-battery warning has already been played this session.
// Resets only on power cycle — intentional, we don't want repeated alerts.
...
void initBattery() {
  ...
  DEBUG_PRINTLN("[BATTERY] Initialized.");
}

// ─── Read average ADC value ────────────────────────────────────────────────────
// ESP32 ADC is noisy. Averaging 64 samples reduces noise significantly.
...
// Returns percentage rounded to nearest 10 (10, 20, ... 100).
int readBatteryPercent() {
  ...
  float batteryVoltage = (pinVoltage * BATTERY_DIVIDER_RATIO) + BATTERY_ADC_OFFSET;

  // Clamp to known battery range
  batteryVoltage = constrain(batteryVoltage, BATTERY_MIN_VOLTAGE, BATTERY_MAX_VOLTAGE);
  ...
  DEBUG_PRINT("[BATTERY] ADC raw: ");
  DEBUG_PRINT(adcRaw);
  DEBUG_PRINT(" | Pin voltage: ");
  DEBUG_PRINT(pinVoltage);
  DEBUG_PRINT("V | Battery: ");
  DEBUG_PRINT(batteryVoltage);
  DEBUG_PRINT("V | Percent: ");
  DEBUG_PRINT(rounded);
  DEBUG_PRINTLN("%");
...
int batteryPercentToAudioTrack(int percent) {
  switch (percent) {
    case 100: return AUDIO_BATTERY_100;
    case 90:  return AUDIO_BATTERY_90;
    case 80:  return AUDIO_BATTERY_80;
    case 70:  return AUDIO_BATTERY_70;
    case 60:  return AUDIO_BATTERY_60;
    case 50:  return AUDIO_BATTERY_50;
    case 40:  return AUDIO_BATTERY_40;
    case 30:  return AUDIO_BATTERY_30;
    case 20:  return AUDIO_BATTERY_20;
    case 10:  return AUDIO_BATTERY_10;
    default:  return AUDIO_BATTERY_10; // Fallback for anything unexpected
  }
}
...
// Called every BATTERY_CHECK_INTERVAL_MS from the main loop. Plays warning audio 
// only once per threshold crossing per session.
