...
// ─── SOS response flag ─────────────────────────────────────────────────────────
// Set by WriteCallbacks when app responds OK or FAIL after SOS
...
// ─── BLE Write Callbacks ───────────────────────────────────────────────────────
// Handles all messages written by the app to the ESP32. 
// Two categories: SOS responses (OK/FAIL) and config messages (CFG:...)
class WriteCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pChar) override {
    String value = pChar->getValue().c_str();
    value.trim();
    DEBUG_PRINT("[BLE] Received from app: ");
    DEBUG_PRINTLN(value);
    ...
    if (value == "OK") {
      ...
      DEBUG_PRINTLN("[BLE] Unknown message — ignored.");
    }
  }
};
...
void initBLE() {
  ...
  DEBUG_PRINTLN("[BLE] Advertising as '" BLE_DEVICE_NAME "'.");
}
...
void skipBLEConnection() {
  bleSkipped = true;
  DEBUG_PRINTLN("[BLE] Skipped by user.");
...
// Called from STATE_BLE_CONNECTING in loop()
// Non-blocking: uses millis() timing so sonar/button still run in parallel. 
// Flow:
...
void handleBLEConnection() {
  ...
  // Connected!
  if (bleConnected) {
    DEBUG_PRINTLN("[BLE] Connection established.");
  ...
  if (millis() - _bleAttemptStartMs >= BLE_CONNECT_TIMEOUT_MS) {
    DEBUG_PRINTLN("[BLE] Connection timeout. Retrying...");
...
// ─── Mid-session reconnect handler ─────────────────────────────────────────────
// Called from STATE_RUNNING in loop()
...
void handleMidSessionReconnect() {
  ...
  DEBUG_PRINTLN("[BLE] Mid-session disconnect. Pausing to reconnect...");
